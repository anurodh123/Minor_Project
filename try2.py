...
def movement_direction(path):
    """Describe the head's starting cylinder and which way it first moves."""
    if len(path) < 2:
        return f"Head at cylinder {path[0]} — no requests to service."
    head, first = path[0], path[1]
    if first > head:
        arrow, word = "\u2192", "right (toward higher cylinders)"
    elif first < head:
        arrow, word = "\u2190", "left (toward lower cylinders)"
    else:
        arrow, word = "\u2022", "no movement (first request is under the head)"
    return f"Head at cylinder {head} {arrow} moving {word}"
...
# `path` is the full physical head trace, starting at the head position and
# including any mandatory boundary bounces (e.g. SCAN/C-SCAN swinging out to
# cylinder 0 or disk_size - 1 even when no request sits there).
# --------------------------------------------------------------------------

def draw_zigzag_trace(ax, name, path, disk_size, total=None, color="#2F5597"):
    positions = path
    ...
    # Highlight the starting head position and its initial direction of travel.
    ax.plot(head, 0, marker="o", markersize=10, markerfacecolor="none",
            markeredgecolor="#C0392B", markeredgewidth=2, zorder=5)
    if len(positions) > 1:
        arrow_dx = disk_size * 0.05 if positions[1] >= head else -disk_size * 0.05
        ax.annotate(
            "", xy=(head + arrow_dx, 0), xytext=(head, 0),
            arrowprops=dict(arrowstyle="-|>", color="#C0392B", lw=2),
            annotation_clip=False,
        )
    ax.annotate(
        f"Head start: {head}", (head, 0), textcoords="offset points",
        xytext=(0, -14), ha="center", fontsize=8, color="#C0392B", fontweight="bold",
    )

    ax.invert_yaxis()
    ax.set_xlim(-disk_size * 0.03, disk_size * 1.03)

    # Ensure the last cylinder (disk_size - 1) always has a visible tick,
    # in addition to the normal auto-generated ticks. Drop disk_size itself
    # (e.g. 200) since it isn't a valid cylinder and only clutters the axis
    # next to the disk_size - 1 tick (e.g. 199).
    ticks = set(ax.get_xticks())
    ticks.discard(disk_size)
    ticks.add(disk_size - 1)
...
class DiskSchedulerWindow(QMainWindow):
    ...
    def _plot_single(self, name, path, disk_size, total):
        ...
        draw_zigzag_trace(ax, name, path, disk_size, total=total)
        self.figure.tight_layout()
        self.canvas.draw()

    def _plot_comparison(self, results, disk_size):
        self.figure.clear()
        names = list(results.keys())
        for i, name in enumerate(names):
            order, total, path = results[name]
    ...
    def run_selected(self):
        ...
        order, total, path = func(requests, head, disk_size=disk_size, direction=direction)

        self.summary_label.setText(
            f"<b>{name}</b> — Service order: {order}<br>"
            f"Total head movement: <b>{total}</b> cylinders<br>"
            f"{movement_direction(path)}"
        )
        self.table.setRowCount(1)
        self.table.setItem(0, 0, QTableWidgetItem(name))
        self.table.setItem(0, 1, QTableWidgetItem(str(total)))
        self._plot_single(name, path, disk_size, total)
    ...
    def compare_all(self):
        ...
        for name, func in ALGORITHMS.items():
            order, total, path = func(requests, head, disk_size=disk_size, direction=direction)
            results[name] = (order, total, path)

        best = min(results, key=lambda n: results[n][1])
        self.summary_label.setText(
            f"Compared all 6 algorithms on queue {requests} from head {head}.<br>"
            f"Best (lowest total movement): <b>{best}</b> ({results[best][1]} cylinders)<br>"
            + "<br>".join(
                f"&nbsp;&nbsp;{n}: {movement_direction(results[n][2])}" for n in results
            )
        )

        self.table.setRowCount(len(results))
        for row, (name, (order, total, path)) in enumerate(results.items()):
            self.table.setItem(row, 0, QTableWidgetItem(name))
            self.table.setItem(row, 1, QTableWidgetItem(str(total)))

        self._plot_comparison(results, disk_size)
